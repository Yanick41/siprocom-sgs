...
import { FiEye, FiEyeOff, FiAlertTriangle, FiCheckCircle } from 'react-icons/fi';
 ...
 * is: the warning at the top is the first thing on it, and the confirmation
 * ends by telling the operator to switch the secret back off. A recovery tool
 * that does not say that is how a break-glass becomes permanent.
 */
    ...
    <main className="flex min-h-screen items-center justify-center bg-slate-100 p-4">
      <div className="w-full max-w-md">
        <div className="card p-6 sm:p-8">
          ...
          <div role="alert" className="mb-5 flex items-start gap-2.5 rounded-lg bg-amber-50 p-3">
          ...
          {submitError && (
            <div role="alert" className="mb-4 rounded-lg bg-red-50 p-3 text-sm text-red-700">
              {/* A 404 here means the secret is wrong or the feature is off. The
                  server cannot tell the difference apart on purpose, so neither
                  can this message. */}
          ...
          <form
            onSubmit={handleSubmit((values) => {
              setSubmitError(null);
              // Stripped rather than sent: the confirmation field is a typo
              // guard for the person, and the server has no use for it.
